...
export const useCartStore = create<CartStore>((set, get) => ({
  ...
  getCartItems: async () => {
    try {
      ...
      const updatedCart = res.data.map((product: CartItem) => {
        ...
        return { 
          ...product,
          cartItemId: product.cartItemId, // Add this line
          finalPrice 
        };
      });
  ...
  addItem: async (product: CartItem) => {
    try {
      ...
      set((state) => {
        // Get the actual cartItemId from the backend response
  ...
  getCoupon: async () => {
    try {
      ...
      toast.error(error.response?.data.message || 'Failed to get valid coupon');
    }
  },
